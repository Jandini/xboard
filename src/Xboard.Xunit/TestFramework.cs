...
using Xboard.Services;
using Xunit.Abstractions;
using Xunit.Sdk;

namespace Xboard;

public class TestFramework(IMessageSink messageSink) : XunitTestFramework(messageSink), IDisposable
{        
    readonly XboardTestRunner _testRunner = new XboardTestRunner(messageSink);
    ...
    {
        private readonly XboardTestRunner _testRunner;
        private readonly IMessageSink _diagnosticMessageSink;

        public TestFrameworkExecutor(AssemblyName assemblyName, ISourceInformationProvider sourceInformationProvider, IMessageSink diagnosticMessageSink, XboardTestRunner testRunner)
    ...
    private class TestAssemblyRunner : XunitTestAssemblyRunner
    {
        ...
        public TestAssemblyRunner(ITestAssembly testAssembly, IEnumerable<IXunitTestCase> testCases, IMessageSink diagnosticMessageSink, IMessageSink executionMessageSink, ITestFrameworkExecutionOptions executionOptions, XboardTestRunner testRunner)
        ...
        protected override string GetTestFrameworkDisplayName() => $"Xboard {_testRunner.Version}";
    }
    ...
    private class TestCollectionRunner : XunitTestCollectionRunner
    {
        ...
        public TestCollectionRunner(ITestCollection testCollection, IEnumerable<IXunitTestCase> testCases, IMessageSink diagnosticMessageSink, IMessageBus messageBus, ITestCaseOrderer testCaseOrderer, ExceptionAggregator aggregator, CancellationTokenSource cancellationTokenSource, XboardTestRunner testRunner)
    ...
    private class TestClassRunner : XunitTestClassRunner
    {
        ...
        public TestClassRunner(ITestClass testClass, IReflectionTypeInfo @class, IEnumerable<IXunitTestCase> testCases, IMessageSink diagnosticMessageSink, IMessageBus messageBus, ITestCaseOrderer testCaseOrderer, ExceptionAggregator aggregator, CancellationTokenSource cancellationTokenSource, IDictionary<Type, object> collectionFixtureMappings, XboardTestRunner testRunner)
    ...
    private class TestMethodRunner : XunitTestMethodRunner
    {
        ...
        public TestMethodRunner(ITestMethod testMethod, IReflectionTypeInfo @class, IReflectionMethodInfo method, IEnumerable<IXunitTestCase> testCases, IMessageSink diagnosticMessageSink, TestResultBus messageBus, ExceptionAggregator aggregator, CancellationTokenSource cancellationTokenSource, object[] constructorArguments, XboardTestRunner testRunner)
